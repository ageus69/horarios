...
#materias = ['I5893','I5894','I5892','I7022']
#generaciones = 10
...
class Horario():
    ...
    def getJson(self):
        ...
        clases_object = {}
        for clase in self.clases:
            clases_object[clase.materia] = {'profe':clase.profe,'nrc':clase.nrc}
            dias_object = {}
            for dia in clase.dias:
                if(hI > dia.horaI):
                    hI = dia.horaI
                if(hF < dia.horaF):
                    hF = dia.horaF
                dias_object[diasMap[dia.dia]] = {'horaI':dia.horaI, 'horaF':dia.horaF}
            clases_object[clase.materia]['dias'] = dias_object
...
if(len(buenos) < 15):
    for i in range(len(buenos)):
        f.write(str(i))
        f.write(' ')
        msg = {'type':'horario','body':json.dumps(buenos[i].getJson()),'key':i}
        sys.stdout.flush() 
        print(json.dumps(msg))
        time.sleep(0.005)
else:
